...
function createSecondMarkUp(data) {
  const largeInfo = data
    .map(({ flags, name, capital, population, languages }) => {
      const box = `<ul class="country-info__list"><li class="country-info__item"><img src="${
        flags.png
      }"alt="The flag of ${
        name.official
      }" class="country-info__img" height="30px" width="60px"><h2 class="country-info__top-text">${
        name.official
      }</h2></li>
        <li class="country-info__item">Capital: <i>${capital}</i></li>
        <li class="country-info__item">Population: <i>${population}</i></li>
        <li class="country-info__item">Languages: <i>${Object.values(
          languages
        )}</i></li></ul>`;
      return box;
    })
